...
1. Create a new branch for your work (optional but recommended):
...
1. After completing the feature, merge it back into `develop`:
...
1. After testing, merge the release branch into `main` and `develop`:
...
1. For urgent fixes, create a hotfix branch from `main`:

```bash
git checkout -b hotfix/<issue> main
```

1. After fixing the issue, merge the hotfix back into `main` and `develop`:
